...
export default new Vuex.Store({
  ...
  getters: {
    ...
    get_lista_stvari: (state) => {
      return state.lista_stvari;
    },
  },
  mutations: {
    ...
    DELETE_LICITACIJU(state, id) {
      const index = state.sve_licitacije.findIndex((item) => item.doc_id == id);
    ...
    nova_licitacija({ commit, getters }, payload) {
      ...
      if (
        ...
        firestore_baza
          .get()
          .then((docExist) => {
            //ako postoji "document" npr "korisnik_id-8" znaci samo update podataka
            if (docExist.exists) {
              // Update to the "licitacije" array field.
              ...
              //ako ne postoji "document" npr "korisnik_id-8" onda mora SET tj napravi document pod tim nazivom
              // add to the "licitacije" array field.
              firestore_baza.set({
                // licitacije: firebase.firestore.FieldValue.arrayUnion({
                random_id: random_id,
                vrsta_licitacije: payload.vrsta_licitacije,
